...
- **SQLite state/outbox/audit**：写入状态、幂等记录、索引任务指针与操作审计；不是正文仓库。
...
- 本机 `/Users/magnetic/hippocampus/` 项目根（D2）：`plan/` 计划文档、`phase0/` 的 Phase 0 源码、配置、测试结果，以及命名为 `hippocampus-phase0-<run_id>` 的 disposable Compose 对象；
- Pi5 `/home/kkp/hippocampus/`、`/home/kkp/.config/hippocampus/`；
- Hippocampus 专属 Compose project、三个容器、内部 network/volumes 和唯一 LAN 映射 `192.168.2.41:8888 → 8080`；
...
- OpenClaw 原生能力不足且需要安装 adapter/skill（此时按 §1.5 之外的变更走独立子计划与新授权）。

禁止 `docker system prune`；禁止覆盖未知目录；禁止重启 Home Assistant、Gotify、Backup API、Caddy 或其他现有容器。需要 OpenClaw adapter 时只生成明确的变更子计划（版本、路径、服务、网络、权限、卸载回滚），取得新授权后再执行。
