...
failpoint/断电模拟分别覆盖：audit started 前后；reservation insert/lease 前后；staging create/write/fsync(file)/fsync(parent) 前后；prepared+audit 提交前后；rename/fsync(parent) 前后；ready+audit 终态前后；Hindsight retain 成功前后；SQLite indexed ack 前后。恢复后：

- [ ] 无丢失、无双正式 MD；
- [ ] 同一 `document_id` 只有一个 current document，replace 后旧 fact set 不残留；同一 retrieval_text 合法提取的多个不同 semantic facts 不按“重复”误判；
- [ ] 旧事件不误标新版本 indexed；
- [ ] 同 client+同 UUIDv4 key+同规范化 payload HMAC 重试复用原事件；同 key+不同 payload 为 409；秘密拒绝不计算/保存 fingerprint；
...
- [ ] prepared 后模拟 staging 与正式文件同时缺失时标记 `conflict/DURABILITY_GAP` 并 No-Go，不静默新建或猜测成功；`readyz=503`、本地安全计数/CLI 可见，且未创建 Gotify/webhook；
...
- [ ] Phase 0 只使用版本化根与 `hippocampus-v4-1-phase0-<run_id>` 对象；计划/派生物 manifest digest 一致，未知或旧版产物会停止；Pi5 零写入，清理未触及其他 Docker 对象；
...
- [ ] `rollback-server.sh` 在 disposable 环境通过 dry-run/execute：错误 project/label 拒绝，绝无 `down -v`，命名卷与数据保留；`rollback-clients.sh` 在合成配置及三端实际备份上完成可逆演练，输出无正文/diff/凭证；

## 10.9 Commissioning 收口

- [ ] Go 前所有三端写入都只使用 `project=commissioning`、隔离 Bank `commissioning-v4-1` 和合成数据；正式 `main`、实际项目与 `global` 保持空；
...
10. JSON-RPC ingress、audit gate/终态、query/idempotency HMAC、导出权限与 sink 故障 fail-closed 通过；
