...
> 属于 [Hippocampus 实施计划 v4.2](00-overview-v4.md) · final · 2026-08-08 · 每步验收标准见 [11-step-acceptance.md](11-step-acceptance.md)

## Phase 0：隔离契约与配置验证（Pi5 只读）

持久工作区固定为 `/Users/magnetic/hippocampus/phase0/v4.2-final-2026-08-08/`；disposable Docker project 固定前缀 `hippocampus-v4-2-phase0-`，其对象固定标签 `io.hippocampus.managed=true`、`io.hippocampus.plan=v4.2-phase0`。开始前校验计划 `MANIFEST.sha256`，在每份模板/报告 manifest 中记录该文件 digest；发现旧版或未知产物停止。允许访问镜像 registry、Hindsight 官方源和用户指定的 cliproxy 做兼容探测，但只产生测试流量/可能费用，不修改 cliproxy 配置；该端点不得是 `.2.41` Claude CLI Gateway。Pi5 只做资源、监听、Docker 管理面和冲突的只读复核。

1. 固定 Markdown schema、URI、工具输入输出、状态机、错误码与审计字段（[07 §7.6](07-mcp-tools-audit.md)）；
2. 实现本地确定性秘密扫描器（内嵌 `scan_policy_version=sp1`）与合成 canary 测试；
...
8. 生成 Compose、非秘密 `hindsight.env`/example、secret plane 模板与测试脚本；验证固定 project/network/volume/mount/labels、强制 service env_file、每个秘密显式映射、DB 用户/库连接、healthcheck、资源上限与 hardening；同时固定 tiktoken `cl100k_base` 计数实现版本并生成测试向量（[03 §3.3](03-data-model.md)）；
9. 生成 remote/Caddy/OAuth example，仅放候选项目目录，不加载现网。
10. 用 disposable MCP 验证 JSON-RPC batch/压缩/超限/canary 拒绝、audit ingress/终态故障（含未认证拒绝行）、reservation 并发与 file+directory fsync failpoint；failpoint 为测试构建内置开关，仅 disposable/测试镜像包含，正式镜像不含该代码路径；确认本机 Claude Code/Codex 不依赖 batch；
11. 只读记录 Mac 到 Pi5 的候选 source 地址与路由依据；它不是自动授权，Phase 1 仍须以实际 socket peer 确认。
12. 在 disposable project 演练 `rollback-server.sh`：先用**正式回滚清单**指向 disposable 环境，必须因 project/labels 不匹配拒绝；再用本 run 生成的 disposable 回滚清单执行 `--dry-run/--execute`，命令不得含 `-v`，down/up 后命名卷数据仍在；用合成客户端配置演练 `rollback-clients.sh` 且输出无正文/diff/凭证。
...
7. 建空的正式 `main` Bank 与隔离 `commissioning-v4-2` Bank，两者均显式锁定 `store_document_text=false`、`audit_log_enabled=false`；MCP 先以 commissioning mode 固定路由后者；部署四工具、reservation/prepared/ready Outbox、单实例 worker、startup recovery 与 audit；MCP 不设 Hindsight healthy 强依赖；
