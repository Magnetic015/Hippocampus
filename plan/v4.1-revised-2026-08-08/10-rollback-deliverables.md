...
- 服务端回滚先核对 project=`hippocampus` 与 `io.hippocampus.managed=true` 标签，再执行不带 `-v` 的精确 Compose down；本机只清理 `hippocampus-v4-1-phase0-<run_id>` disposable project；
...
│   ├── rollback-clients.sh
│   └── rollback-server.sh      # 固定 project/labels，禁止 down -v
...
├── plan/
│   └── v4.1-revised-2026-08-08/  # 本独立实施版本集（00–10 + MANIFEST.sha256）
└── phase0/
    └── v4.1-revised-2026-08-08/  # 仅本版 Phase 0 隔离验证工作区
        ├── scanner/                # 秘密扫描器 + canary 测试
        ├── hindsight-lab/          # disposable compose（前缀 hippocampus-v4-1-phase0-）
