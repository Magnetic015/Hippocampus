# 06 · Docker、网络与 Hindsight 配置（v4.2）
...
- project/network/volume/service 均加 `io.hippocampus.managed=true`、`io.hippocampus.plan=v4.2` 标签；Phase 0 disposable 对象（前缀 `hippocampus-v4-2-phase0-`）固定标签 `io.hippocampus.managed=true`、`io.hippocampus.plan=v4.2-phase0`，与正式回滚清单天然不互配（见 [10 §12](10-rollback-deliverables.md)）；回滚一律按环境回滚清单对 project 与标签双重核对；
