# 07 · Hippocampus MCP 一期工具契约与操作审计（v4.1）
...
- `/mcp/` 的 POST/GET/DELETE 均先认证，再解析 body 或查询 session；客户端自报 request-id Header 永远不作为服务端 ID；
...
字段上限：title 256 Unicode scalar、summary 60–200 中文字符、retrieval_text 100–600 tokens（硬上限 800）、detail_body 128 KiB、event_at/project/type 按固定 schema；渲染后完整 Markdown 不超过 192 KiB，整个 JSON-RPC body 仍受 §7.0 的 256 KiB 上限。超限在 reservation/文件写入前拒绝。
...
- 存储：SQLite `audit` 表（与 outbox 同库不同表），`request_id UNIQUE`；`CHECK(state IN started,prepared,stored_pending,ok,idempotent_replay,rejected,error,interrupted,recovery_pending)`；普通主路径 `started→ok`，commit 可走 `started→prepared→stored_pending→ok`，其余只走列出的终态；
- 每个已认证 MCP HTTP request 恰有一行。初始只写 `ts, request_id, process_instance_id, client_id, state=started`，`operation_enum/tool_enum/root_request_id/event_id` 均为空；不得含任何客户端字段。envelope 通过后才写 allowlisted `operation_enum`，tools/call 只写四个 canonical `tool_enum` 或 `invalid_tool`；
- 完整安全字段：`ts, request_id, process_instance_id, root_request_id|null, event_id|null, client_id, operation_enum, tool_enum|null, normalized_document_id|uri|null, state, outcome_code, latency_ms, scan_policy_version`。首次 commit 的 `root_request_id=request_id`；后续每次重放保留自己的 request_id 并指向既有 root/event；非 commit 两列为空；在保留期内形成 `1 event : N audit requests`；
