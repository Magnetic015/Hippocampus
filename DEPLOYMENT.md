...
| hindsight | `hippocampus-hindsight-1` | `ghcr.io/vectorize-io/hindsight@sha256:${HINDSIGHT_DIGEST}` | `192.168.2.41:9999→9999`(控制面) | `hippocampus-hf-cache` | 3 GiB |
...
- **对外 8888 + 9999**;Hindsight 原生 API(容器内 **8888**,非 8000)、Hindsight 原生 MCP、db 5432 **均不发布**。控制面自 2026-08-09 起 `HINDSIGHT_ENABLE_CP=true` 并发布 `9999`,**自身无任何认证**——同网段任何设备可读写整库并跑 reflect,绕过 MCP 授权面(书面接受,见 §13)。
...
`STORE_DOCUMENT_TEXT=false`、`ENABLE_AUTO_CONSOLIDATION=false`、`MCP_ENABLED=false`、`ENABLE_API=true`、**`ENABLE_CP=true`**、`LLM_TRACE_ENABLED=false`、`OTEL_TRACES_ENABLED=false`、`AUDIT_LOG_ENABLED=false`;`LLM_OUTPUT_LANGUAGE=Chinese`、`QUERY_ANALYZER_LANGUAGES=en,zh`;retain 模型见 §13(现网 primary=`gpt-5.6-terra`、backup=`gpt-5.6-sol`);**reflect 模型 primary=`gpt-5.5`、backup=`gpt-5.6-sol`**(见 §13);embedding=`paraphrase-multilingual-MiniLM-L12-v2`;reranker=`cross-encoder/mmarco-mMiniLMv2-L12-H384-v1`(fallback `rrf`);`RERANKER_MAX_CANDIDATES=80`(**延迟瓶颈,见 §13**)。数字 LLM 成员**不继承** primary 的 key。

reflect 主成员的 `api_key`/`base_url` 会回退到全局 LLM(`memory_engine.py:1470-1472`),故只需在 `hindsight.env` 写 `REFLECT_LLM_PROVIDER`/`MODEL`;但**索引成员什么都不继承**,`REFLECT_LLM_1_BASE_URL`/`_API_KEY` 必须在 `compose.yaml` 显式映射,否则 `_parse_llm_members` 在启动时直接抛 `ValueError`。
...
- **搜索/reranker 延迟——当前不达 §10.4**:search p50≈8.96s、**p95≈11.3s**(目标 ≤5s;read/commit/status 皆毫秒)。根因:Pi5 ARM CPU 上 reranker 占召回 ~99%。可逆 A/B:`RERANKER_MAX_CANDIDATES` 100→20 → 2.1–2.3s;**现网已定为 80**,该值下的中文召回质量尚未重验。**待决**:继续降候选数 + 重验中文召回 / 重批 p95 阈值到实测基线 / Phase 3 换 `rrf`·PGroonga·VectorChord。
...
- **retain 模型漂移**:计划 `gemini-3-flash`/`deepseek-v4-flash` 在 cliproxy 上不可用/不支持 json_schema;现网 primary 已切 **`gpt-5.6-terra`**(对应 commit `bc9cf8b`)。backup 原为 `gemini-3.6-flash`,2026-08-09 实测其**返回 403「billing 未启用」,连普通 chat 都失败**,即 failover 备份长期是死的;已改为 **`gpt-5.6-sol`**。`gemini-3.5-flash` 同样 403,但 `gemini-3-flash-agent`/`gemini-3.1-flash-lite` 正常——是这两个别名背后的上游项目问题,不是 gemini 整体不可用。
- **CP 控制面 LAN 无认证——书面接受**:`HINDSIGHT_ENABLE_CP=true` 且发布 `192.168.2.41:9999`,CP 自身无登录。实测未带任何凭据即可 `GET /api/banks` 拿到全部 bank 与 `fact_count`,并可经 CP 页面跑 recall/reflect,**完全绕过** MCP 的 Bearer + peer IP 绑定 + 服务端二次授权 + 输出扫描,与 `07-mcp-tools-audit.md` §「agent 直连 Hindsight」红线相冲突。经 2026-08-09 决策**书面接受该残余风险**,与「LAN 明文」并列。可逆缓解:端口改绑 `127.0.0.1:9999` + SSH 隧道。
- **reflect 模型准入(Phase 3,2026-08-09 授权)**:primary **`gpt-5.5`**、backup **`gpt-5.6-sol`**,两者均通过 tool-calling + json_schema 且模型身份未被改路由;以「主成员指向不存在模型」演练,日志确认 `LLM member 0 … failed on call_with_tools; trying next member`,备份**真实接管**且答案正确。落选:`glm-5.2`(plan 另一候选,cliproxy 上不存在)、`gemini-3.5/3.6-flash`(403 billing)、`deepseek-v4-pro`(不支持 json_schema)、`claude-5.5`/`claude-5.6`(被 cliproxy 静默改路由到 `gpt-5.5`/`gpt-5.6-sol`)。⚠️ **该 cliproxy 上不存在真正跨厂商备份**,reflect 与 retain 的 failover 均局限在 gpt 族内,上游整体故障时无冗余。⚠️ 演练还暴露:主成员挂掉时,agent 循环的**每一次** tool call 都会先重试主成员 4 次再回退,reflect 整体耗时显著拉长。
- **现网硬化漂移(部分已修)**:启用 CP 时手工改动的 `compose.yaml` 曾删掉 hindsight 的 `user`/`read_only`/`tmpfs`/`cap_drop`/`healthcheck`(这正是 `docker ps` 里 hindsight 独缺 `(healthy)` 的原因),2026-08-09 已全部回补并验证(`docker diff` 证实 CP 对 `/app/control-plane` 零写入,`read_only` 安全)。**残留**:`db` 服务的 `cap_drop: [ALL]` 仍缺失,仓库模板正确、现网未回补。
- **`HINDSIGHT_INTERNAL_TOKEN` 暴露**:2026-08-09 排障时经 `docker exec … env` 读出明文,已进入对话通道;该令牌同时用作 `HINDSIGHT_API_TENANT_API_KEY` 与 `HINDSIGHT_CP_DATAPLANE_API_KEY`。**建议纳入轮换**(需同步 hindsight 与 MCP 两侧并重建容器),与 cliproxy key 一并处理。
...
## 14. 当前状态快照 (2026-08-09)

- 三容器 healthy(hindsight 的 `healthcheck` 已回补,不再是无健康态的 `Up`);MCP 镜像 = **`hippocampus-mcp:v4.2-metafix`**(含 `_meta` 信封修复);db/hindsight digest 固定运行。
- `HIPPOCAMPUS_MODE=production`;Bank `main` 208 facts(生产 project `soul`),Bank `commissioning-v4-2` 338 facts 作为不可见回归集。
- 本机 Claude 经 MCP 真实 `memory_search` **已验证可用**(project `soul`);Hindsight 原生 API 监听 **8888**,MCP 侧 `HIPPOCAMPUS_HINDSIGHT_BASE=http://hindsight:8888`。
- **reflect 已启用**:`gpt-5.5`(备 `gpt-5.6-sol`),对 `main` 实测返回正确中文综述(budget=low、54k in/4.1k out、约 118s);CP 控制面 `9999` 可用但无认证(见 §13)。
- 仓库模板已与现网对齐:`templates/compose.yaml` 渲染出的 hindsight 服务与现网**逐字节一致**,`templates/hindsight.env` 设置项完全一致。
- Go/No-Go **未评估**;§10.4 延迟(现 `RERANKER_MAX_CANDIDATES=80` 未重验中文召回)、TLS/明文、CP 无认证、token 轮换为主要未决项。
- 备份点:`server.py.bak.20260808-215926`、`hippocampus.env.bak.20260808-220050`、`compose.yaml.bak.20260809-164226`、`hindsight.env.bak.20260809-164226`、旧镜像 `v4.2-5cee863` 保留。
